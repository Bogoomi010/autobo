...
세션(스캔 창)은 `BotSettings.scanWindow`로 사용자가 지정한다(시작시각 KST + 지속시간(분),
평일만, 최대 1440분=24시간). 이 창은 **신규 매수 스캔**과(단타봇에 한해) **매도 판정 마감**을
동시에 규정한다. 장투봇은 스캔 창을 매수 스캔에만 쓰고, 매도는 최소 24시간 규칙만 따른다.
