...
매도 후 순실현손익은 다음 라운드 원금에 합산해 복리 재투자한다. 원금은 봇별로 독립적으로
최소 6,000원~최대 100,000원 범위를 지킨다:
- **모의 모드**: `realized > 0`이면 `store.creditVaultFromBot(realized)`로 정산기 돈뭉치
  연출 없이 곧바로 `simBalance`에 합산하고, 같은 순이익을 다음 매수 원금에도 더한다.
  `EV.BOT_PROFIT_CREDITED`를 emit해 `botFloor.ts`가 그 봇 책상 위에 "+₩N" 획득 텍스트를 띄운다.
  손실은 원금에서 차감하되 최소 6,000원 아래로는 내려가지 않는다.
- **실거래 모드**: 매도 주문 자체가 실계좌에서 체결되므로 돈은 이미 계좌에 들어가 있다.
  `executeBuy`/`executeSell` 양쪽에서 `store.refreshAccounts()`를 호출해 계좌 표시를 갱신하고,
  다음 주문 예산에도 직전 순실현손익을 반영한다.
...
|---|---|---|
| 매도 판정 시작 조건 | 세션(스캔 창) 안에서만 | 손절·익절은 즉시, 붕괴 신호는 매수 24시간 후 |
| 세션/기간이 끝나면 | 보유 중이어도 강제 매도(`reason: "timeout"`) | 설정한 최대 보유기간(1~30일)에 강제 매도(`reason: "timeout"`) |
| 24시간 전 신호(TP/SL/붕괴 스코어) | 세션 중이면 그대로 반영 | 손절·익절은 즉시 반영, 붕괴 신호는 대기 |
...
입력한 손절률과 익절률은 보유시간과 무관하게 즉시 적용한다. 붕괴 신호만 매수 24시간 이후부터
평가하며, 신호가 없더라도 최대 보유기간에 도달하면 강제 매도한다. 따라서 장투봇의 24시간은
최소 보유 보장이 아니라 붕괴 신호 판정의 대기시간이다. 매도 후 원금과 순실현손익은 다음 매수에
재투자한다.
...
1. 매도 판정이 열려 있는가?
   ...
   - 장투봇: 손절·익절이 아니고 매수 후 24시간 전인가? → 붕괴 신호 판정 없이 보유
2. 고정 익절: pnl ≥ takeProfitRate → 즉시 매도(reason="profit")
...
두 봇 모두 즉시 적용되는 고정 청산 조건이다. 장투봇도 익절 목표에 도달하면 즉시 매도하고,
24시간 대기 게이트는 붕괴 신호에만 적용한다.
